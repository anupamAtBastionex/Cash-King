...
if(isset($user_signup) && isset($access_key)) {
    ...
    if($key != $access_key){
		...
		$check_exits_user_query = "SELECT email FROM users WHERE email='$email'";
	...
	$register_user = "INSERT INTO users (phone,image,username,password,name,refer,email,device) VALUES ('$phone','$image','$username','$password','$name','$refer','$email','$device')";
	...
	if($send_server){
	    ...
	    $response['error'] = true;
	    $response['message'] = "Something Gone Wrong try Again".mysqli_error($link);
	}
	print_r(json_encode($response));
}else
...
if(isset($user_login) && isset($access_key)) {
	...
	$user_query = "SELECT * FROM users WHERE email='$phone' AND password='$password'";
	...
	if(mysqli_num_rows($sql)==1){
		...
		while($raw = mysqli_fetch_assoc($send_reward)){
         ...
         if($raw['id']==$id)
         {
        	...
        	$response['error'] = "true";
			$response['message'] = "Wrong email / password! Try Again";
	}
...
if(isset($access_key) && isset($add_spin))
{
	...
	if($key != $access_key)
	{
		...
		if(!empty($username) && !empty($type)){
		    ...
		    $time = date("h:i a");
		    //Add Data Tracker
		    $tracker_query = mysqli_query($link, 'INSERT INTO `tracker`(`username`, `points`, `type`, `date`,`time`, `visit_link_id`) VALUES ("'.$username.'", "'.$points.'", "'.$type.'", "'.$date.'", "'.$time.'", "'.$visited_link_id.'")');
...
if(isset($access_key) && isset($check_visit)){
	...
	if($key != $access_key){
		...
		//if(isset($username) && !empty($username) && $username != 'root')
		if(isset($username) && !empty($username))
		{
			//echo $username.'';die;
		    $date = date('Y-m-d');
		    //GET Data Tracker
		    $userVisitedLink = mysqli_query($link, "SELECT `username`, `points`, `type`, `date`, `time`, `visit_link_id` FROM `tracker` WHERE `username` ='".$username."' AND `date`='".$date."'");

		    $visitedRecords = [];
		    if(mysqli_num_rows($userVisitedLink) > 0)
		    {
		    	while($rs = mysqli_fetch_assoc($userVisitedLink))
		    	{
		    		$visitedRecords[] = $rs;
		    	}
		    	$response['error'] 		= false;
			    $response['message'] 	= "Today Visited Records";
		    	$response['data'] 		= $visitedRecords;
		    }else{
		    	$response['error'] 		= false;
			    $response['message'] 	= "No Record Found Today";
		    	$response['data'] 		= $visitedRecords;
		    }
		} else {
		    	$response['error'] = true;
...
if(isset($access_key) && isset($user_tracker)){
	...
	if($key != $access_key){
		...
		$sql = mysqli_query($link,"SELECT MAX(id) as id, date FROM `tracker` WHERE username='".$username."' and points=0 and type='claim' GROUP BY id");
		$result = mysqli_fetch_assoc($sql);

		if(empty($result) || $result['id'] == ''){
			$sql = mysqli_query($link,"SELECT Min(id) as id,date FROM `tracker` WHERE username='".$username."' GROUP BY id");
			$result = mysqli_fetch_assoc($sql);
		}
...
if(isset($id) && isset($access_key) && isset($redeem)&& isset($p_details)) {
	...
	//print_r(json_decode($_POST['p_details'], true));die;
    $response = array();
    if($key != $access_key){
		$response['error'] = "true";
		$response['message'] = "Invalid Access Key";
            	...
            	if(mysqli_num_rows($send_package)>0) 
            	{
                	...
                	if(mysqli_num_rows($amount_sql)>0) 
                	{
	            	    ...
	            	    if($avai_coins>=$coins)
	            	    {
							...
							if($db)
							{
								$p_details = json_decode($_POST['p_details'], true);
//								print_r($p_details['type']);die;

								if($p_details['type'] == "bank")
								{
									$bank_name 	= $p_details['bank_name'];
									$account_no = $p_details['acc'];
									$ifsc_code 	= $p_details['ifsc'];

									$query = "UPDATE users SET bank_name='$bank_name', user_bank_ac_no='$account_no', user_ifsc_code='$ifsc_code', account_update_date='".date('Y-m-d H:i:s')."' WHERE id='$uid'";
									mysqli_query($link, $query);
									$p_details = $bank_name."|".$account_no."|".$ifsc_code;
								}
								else if($p_details['type'] == "upi")
								{
									$upi_id = $p_details['id'];
									$query  = "UPDATE users SET user_upi_id='$upi_id', account_update_date='".date('Y-m-d H:i:s')."' WHERE id='$uid'";
									mysqli_query($link, $query);
						    	...
						    	$register_user = "INSERT INTO reward_list (u_id, package_name, p_details, coins_used, symbol, amount, date, time, status, package_id) VALUES ('$uid','$name','$p_details','$coins','$symb','$amount','$date','$time','0','$p_id')";

	                            $send_server= mysqli_query($link, $register_user);

	                            $get_users = "SELECT * FROM users WHERE id=".$_POST['id'];
								$userData = mysqli_query($link,$get_users);
...
if(isset($scratch) && isset($access_key) && isset($usernamee)) {
    $response = array();
    if($key != $access_key){
		$response['error'] = "true";
		$response['message'] = "Invalid Access Key";
	...
	$get_offers = "SELECT scratch_limit FROM settings";
	$send_offers = mysqli_query($link,$get_offers);
...
//	$r_row = mysqli_fetch_array($r_url)
		if(mysqli_num_rows($send_offers)>0) {
		    $date =date('Y-m-d');
		    $sql= mysqli_query($link,"SELECT * FROM `tracker` where (`type`='Scratch & Win Reward') and `username` = '$usernamee' and date(`date`)='".date('Y-m-d')."'");
		$res = mysqli_fetch_assoc($sql);
		$tracker = mysqli_num_rows($sql);


		    $response['error'] = "false";
			$response['daily'] = $result['scratch_limit'];
			$response['left'] = $tracker;
...
if(isset($video) && isset($access_key) && isset($usernamee)) {
    $response = array();
    if($key != $access_key){
		$response['error'] = "true";
		$response['message'] = "Invalid Access Key";
	...
	$get_offers = "SELECT daily_video_limit FROM settings";
	$send_offers = mysqli_query($link,$get_offers);
...
//	$r_row = mysqli_fetch_array($r_url)
		if(mysqli_num_rows($send_offers)>0) {
		    $date =date('Y-m-d');
		    $sql= mysqli_query($link,"SELECT * FROM `tracker` where (`type`='$videoType') and `username` = '$usernamee' and date(`date`)='".date('Y-m-d')."'");
		$res = mysqli_fetch_assoc($sql);
		$tracker = mysqli_num_rows($sql);


		    $response['error'] = "false";
			$response['daily'] = $result['daily_video_limit'];
			$response['left'] = $tracker;
...
if (isset($c) && isset($access_key)) {
    ...
    $get_reward = "SELECT * FROM settings";
    $send_reward = mysqli_query($link, $get_reward);
    if (c()) {
    $response["error"] = "1";
} else {
    ...
    $response["vpn"] = $raw["vpn"];
    $response['data'] = $raw;
    print_r(json_encode($response));
}

if (isset($check_zone) && isset($access_key)) {
    $response = array(); 
   if ($key != $access_key) {
    $response["error"] = "true";
    $response["message"] = "Invalid Access Key";
   ...
   $get_offers = "SELECT games FROM settings";
$send_offers = mysqli_query($link, $get_offers);

$sql = mysqli_query($link, "SELECT * FROM `tracker` WHERE (`type`='GameZone' or `type`='tier') and `username`='{$check_zone}' and date(`date`)= '" . date("Y-m-d") . "'");

$res = mysqli_fetch_assoc($sql);
$result = mysqli_fetch_assoc($send_offers);

if (mysqli_num_rows($send_offers) > 0) {
    if (mysqli_num_rows($sql) >= $result["games"]) {
        $response["error"] = "true";
        $response["msg"] = "Your daily limit has been reached, To play games come tomorrow or you can play for only fun!";
    } else {
        $response["error"] = "false";
...
if (isset($redeem_check) && isset($access_key)) {
    $response = array(); 
   if ($key != $access_key) {
    $response["error"] = "true";
    $response["message"] = "Invalid Access Key";
...
$user = array();
$get_reward = "SELECT * FROM reward_list WHERE u_id = ".$redeem_check;
//print_r($get_reward);die;
$send_reward = mysqli_query($link, $get_reward);
while ($raw = mysqli_fetch_assoc($send_reward)) {
    $index["id"] = $raw["id"];
    $index["u_id"] = $raw["u_id"];
    $index["package_name"] = $raw["package_name"];
    $index["p_details"] = $raw["p_details"];
    $index["coins_used"] = $raw["coins_used"];
    $index["symbol"] = $raw["symbol"];
    $index["amount"] = $raw["amount"];
    $index["time"] = $raw["time"];
    $index["date"] = $raw["date"];
    $index["status"] = $raw["status"];
    $index["package_id"] = $raw["package_id"];
    $get_img = "SELECT * FROM reward WHERE id =" . $raw["package_id"];
    $send_img = mysqli_query($link, $get_img);
    $img = mysqli_fetch_assoc($send_img);
    $index["image"] = $img["image"];
    array_push($user, $index);
}
if (mysqli_num_rows($send_reward) > 0) {
    $response["error"] = false;
...
//update_account() - update user profile
if(isset($access_key) && isset($update_account))
{
	if($key != $access_key)
	{
		$response['error'] = "true";
	...
	if(!empty($username) && (!empty($upi_id) || (!empty($account_no) && !empty($ifsc_code) && !empty($bank_name))))
	{
		$select = mysqli_query($link, "SELECT * FROM users WHERE username='$username'");
		$data 	= mysqli_fetch_assoc($select);
		if(!empty($data))
		{
			if(!empty($bank_name) && !empty($account_no) && !empty($ifsc_code) && !empty($upi_id))
			{
				$query = "UPDATE users SET bank_name='$bank_name', user_upi_id='$upi_id', user_bank_ac_no='$account_no', user_ifsc_code='$ifsc_code', account_update_date='".date('Y-m-d H:i:s')."' WHERE username='$username'";
			}
			else if(!empty($bank_name) && !empty($account_no) && !empty($ifsc_code))
			{
				$query = "UPDATE users SET bank_name='$bank_name', user_bank_ac_no='$account_no', user_ifsc_code='$ifsc_code', account_update_date='".date('Y-m-d H:i:s')."' WHERE username='$username'";
			}
			else if(!empty($upi_id))
			{
				$query = "UPDATE users SET user_upi_id='$upi_id', account_update_date='".date('Y-m-d H:i:s')."' WHERE username='$username'";
			}else{

				$response['error'] 		= "false";
			    $response['message'] 	= "No change in record! Fill all Bank Details as bank_name, account_no, ifsc_code";
			}
			//echo $query;die;
			...
			{
			    $response['error'] 		= "false";
			    $response['message'] 	= "No change in record! Either same data or some thing went wrong";
			}
		}
		else 
		{
	...
	{
	    $response['error'] = "true";
	    $response['message'] = "Username with (UPI Id) OR (Bank Name, Account, IFSC) Required!";
	}
	print_r(json_encode($response));
}
